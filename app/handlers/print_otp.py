...
from app.states import PrintOtpFlow

router = Router(name="print_otp")

FAN_PATTERN = re.compile(r"^\d{10,16}$")


@router.callback_query(F.data == "menu:print_otp")
async def start_print_otp(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.set_state(PrintOtpFlow.waiting_fan)
    await callback.message.answer(
...
@router.message(PrintOtpFlow.waiting_fan)
async def receive_fan(message: Message, state: FSMContext) -> None:
    ...
    async with SessionLocal() as session:
        ...
        order = await create_order(
            session,
            user_id=user.id,
            order_type=OrderType.PRINT_OTP,
            amount_birr=settings.price_print_otp,
            fan_last4=fan[-4:],
        )
...
async def receive_otp(message: Message, state: FSMContext) -> None:
    ...
    async with SessionLocal() as session:
        user = await get_or_create_user(session, message.from_user.id, message.from_user.username)
        price = settings.price_print_otp
    ...
    pdf_bytes = build_print_sheet_pdf(profile)
    await message.answer_document(
        BufferedInputFile(pdf_bytes, filename="fayda_print_sheet.pdf"),
        caption="✅ Print-ready sheet generated. Send this to any print shop.",
        reply_markup=MAIN_MENU,
    )
    await state.clear()
